...
export default function Insert() {
  ...
  async function insertData(e) {
    ...
    //빈 문자열은 null로 변환해서 전송
    const payload = Object.fromEntries(
      Object.entries(portfolio).map(([key, value]) => [key, value === "" ? null : value]),
    );

    //2. portfolio 테이블 저장
    const { data: insertedPortfolio, error } = await supabase
      .from("portfolio")
      .insert({ ...payload, thumbnail: thumbnailPath }) //글 등록
      .select("id") //등록한 글의 id 조회
      .single();

    if (error) {
      console.error("글 등록 실패:", error.message, error.details, error.hint, error.code);
      //thumbnail 파일 삭제
      if (thumbnailPath) {
        await supabase.storage.from("portfolio").remove([thumbnailPath]);
      }
      alert(`글 등록 실패:${error.message}`);
      return; //실패시 중단 - 없으면 아래에서 insertedPortfolio가 null이라 에러
    }
    ...
    if (imageRows.length > 0) {
      ...
      if (imageError) {
        ...
        alert(`대표 이미지 입력 실패:${imageError.message}`);
        return; //실패했으므로 성공 처리로 넘어가지 않음
      }
    }
    //글 등록 성공시 - 모든 입력값 초기화
  ...
  const handleLogin = async e => {
    ...
    if (error) {
      alert(`로그인 실패:${error.message}`); //alert은 인자를 하나만 표시함
    } else {
      alert("로그인 성공");
      setUser(data.user);
      router.refresh();
